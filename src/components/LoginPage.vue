...
const handleSubmit = () => {
  ...
  const validUser = storedUsers.find(user => user.email === loginForm.email && user.password === loginForm.password)
  if (validUser) {
    localStorage.setItem('currentUser', JSON.stringify(validUser))
    toast.add({ severity: "success", summary: "Login Successful", detail: `Welcome, ${validUser.name}`, life: 3000 })
    router.push("/home")
  } else {
